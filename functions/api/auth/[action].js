...
async function sendMail(env, toEmail, token, purpose) {
  ...
  if (purpose === 'activation') {
      ...
      html = `<div style="font-family:sans-serif; padding:20px; color:#333;"><h2>Satu Langkah Lagi!</h2><p>Klik tombol di bawah ini untuk mengaktifkan akun Anda:</p><br><a href="${host}/?activation_token=${token}" style="background-color:#5b83e8; color:white; padding:12px 24px; text-decoration:none; border-radius:5px; font-weight:bold; display:inline-block;">Aktifkan Akun Saya</a><br><br><p style="font-size:12px; color:#888; margin-top:20px;">Atau copy link berikut: <br>${host}/?activation_token=${token}</p></div>`;
  } else if (purpose === 'reset') {
      subject = "Reset Password Orland Management";
      html = `<div style="font-family:sans-serif; padding:20px; color:#333;"><h2>Reset Password</h2><p>Kami menerima permintaan untuk mereset password Anda. Link ini berlaku selama 30 Menit.</p><br><a href="${host}/?reset_token=${token}" style="background-color:#ef4444; color:white; padding:12px 24px; text-decoration:none; border-radius:5px; font-weight:bold; display:inline-block;">Reset Password Sekarang</a><br><br><p style="font-size:12px; color:#888; margin-top:20px;">Atau copy link berikut: <br>${host}/?reset_token=${token}</p></div>`;
  } else {
      subject = "Kode OTP Orland Management";
      html = `<div style="font-family:sans-serif; padding:20px; color:#333;"><h2>Verifikasi Keamanan</h2><p>Berikut kode OTP Anda (Berlaku 3 Menit):</p><h1 style="letter-spacing:5px; color:#5b83e8;">${token}</h1></div>`;
  }
  ...
  try {
    ...
    if (!res.ok) { const err = await res.json(); console.log("Resend Error:", err); return { success: false }; } 
    return { success: true };
  } catch (e) { console.log("Fetch Error:", e); return { success: false }; }
}
...
export async function onRequestPost({ request, env, params }) {
  try {
    ...
    const now = Math.floor(Date.now() / 1000); // Waktu seragam dalam EPOCH Detik
    ...
    try {
      ...
      if (rl && rl.count >= 6) return jsonError("Terlalu banyak percobaan. IP diblokir sementara.", 429);
    ...
    if (action === "register") {
      ...
      const tokenUUID = crypto.randomUUID().replace(/-/g, '') + crypto.randomUUID().replace(/-/g, ''); // Token bersih tanpa strip
      await env.DB.prepare("INSERT INTO otp_requests (id, identifier, code, purpose, expires_at) VALUES (?,?,?,?,?)").bind(crypto.randomUUID(), body.email, tokenUUID, 'activation', now + 86400).run();
      
      const mail = await sendMail(env, body.email, tokenUUID, 'activation');
      return jsonOk({ message: mail.success ? "Link Aktivasi telah dikirim ke Email Anda." : "Registrasi sukses, namun Sistem Email sedang gangguan.", sim_token: mail.success ? undefined : tokenUUID });
    }
    ...
    // === LUPA PASSWORD (MAGIC LINK RESET 30 MENIT) ===
    if (action === "request-reset") {
      ...
      await env.DB.prepare("INSERT INTO otp_requests (id, identifier, code, purpose, expires_at) VALUES (?,?,?,?,?)").bind(crypto.randomUUID(), user.email, tokenUUID, 'reset', now + 1800).run(); // FIX: 30 Menit murni
      
      const mail = await sendMail(env, user.email, tokenUUID, 'reset');
      return jsonOk({ message: mail.success ? "Tautan Reset Password telah dikirim ke Email Anda." : "Sistem email gangguan.", sim_token: mail.success ? undefined : tokenUUID });
    }

    // === SUBMIT PASSWORD BARU ===
    if (action === "reset-password") {
      // Pastikan token valid dan belum lewat batas waktu
      ...
      if(!tokenRow) return jsonError("Tautan Reset sudah kadaluarsa (Lebih dari 30 Menit) atau tidak valid.");
    ...
    if (action === "login-password") {
      ...
      if(user.status !== 'active') return jsonError("Akun belum diaktifkan. Silakan cek email Anda untuk Link Aktivasi.", 403);
    ...
    if (action === "login-pin") {
      ...
      if (user.pin_hash !== hashInput) {
        ...
        if (fails >= 3) {
          await env.DB.prepare("UPDATE users SET fail_count=?, locked_until=? WHERE id=?").bind(fails, now + 900, user.id).run();
          return jsonError("PIN Salah 3x. Akun dikunci 15 menit.", 429);
        }
        await env.DB.prepare("UPDATE users SET fail_count=? WHERE id=?").bind(fails, user.id).run();

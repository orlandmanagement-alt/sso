...
export async function registerService({ env, body }){
  ...
  if(!["talent", "client"].includes(role)){
    return jsonInvalid({ message: "role_must_be_talent_or_client" });
  }

  if(!email || !email.includes("@") || !wa || password.length < 8){
    return jsonInvalid({ message: "invalid_register_payload" });
  }

  const exists = await findUserByEmail(env, email);
  if(exists?.id){
    return jsonConflict({ message: "email_already_used" });
  }
  ...
  try{
    ...
    return jsonError({
      message: "failed_to_start_registration",
...
export async function verifyRegisterService({ env, body }){
  ...
  if(!["talent", "client"].includes(role) || !email || !wa || !password || !otp){
    return jsonInvalid({ message: "invalid_verify_register_payload" });
  }
  ...
  try{
    await createUser(env, {
      id: user_id,
      email_norm: email,
      display_name: email.split("@")[0] || role,
      status: "active",
      phone: wa,
      password_hash: hash,
      password_salt: salt,

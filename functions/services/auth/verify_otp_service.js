...
import { nowSec, sha256Base64, defaultPortalFromRoles, buildPortalRedirectUrl, deniedUrl, makeSessionCookie } from "./shared.js";
...
export async function onVerifyOtp({ request, env, body }){
  ...
  if(!isOtpFormat(otp)) return jsonInvalid({ message: "otp_invalid" });
  ...
  if(requestedPortal && !access.has(requestedPortal) && !["super_admin","admin","staff","security_admin"].some(x => access.has(x))){
    return jsonForbidden({ message: "role_not_allowed_for_portal", portal: requestedPortal, denied_url: deniedUrl(env, "role_not_allowed", requestedPortal, next) });
  }
  ...
  try{
    await createSession(env, {
      id: sid, user_id: user.id, token_hash: sid, created_at: now, expires_at: now + ttlSec,
      revoked_at: null, ip_hash: ip_hash, ua_hash: ua_hash, role_snapshot: JSON.stringify(roles || []),
      ip_prefix_hash: null, last_seen_at: now, roles_json: JSON.stringify(roles || []),
      session_version: 1, revoke_reason: null
    });
    return jsonOk({ verified: true, sid, portal, roles, redirect_url: buildPortalRedirectUrl(env, portal, next) }, { "set-cookie": makeSessionCookie(request, env, sid, ttlSec) });
  }catch(err){

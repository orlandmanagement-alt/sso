...
export async function onMe({ request, env }){
  ...
  return jsonOk({ user_id: a.uid, roles: a.roles || [] });
}
...
export async function onLogout({ request, env }){
  const a = await requireSessionAuth(env, request);
  if(a.ok) await revokeSession(env, a.sid, nowSec(), "logout");
  return jsonOk({ logged_out: true }, { "set-cookie": clearSessionCookie(request, env) });
}
...
export async function onLogoutAll({ request, env }){
  ...
  return jsonOk({ logged_out_all: true }, { "set-cookie": clearSessionCookie(request, env) });
}
...
export async function onRefresh({ request, env }){
  const a = await requireSessionAuth(env, request);
  if(!a.ok) return jsonUnauthorized({ message: "unauthorized" });
  return jsonOk({ refreshed: true, user_id: a.uid, roles: a.roles || [] });
}
